...
export function calculateReturns(stocks: { ticker: string; data: { changePercent: number }[] }[]) {
  return stocks.map((stock) =>
    stock.data.map((d) => d.changePercent / 100) // Convert percent to decimal
  );
}
...
export function calculateCorrelation(returns: number[][]) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const stdDevI = sqrt(returns[i].reduce((sum, x) => 
      sum + Math.pow(x - meanI, 2), 0) / (returns[i].length - 1));

    for (let j = 0; j < n; j++) {
      const meanJ = mean(returns[j]);
      // Calculate standard deviation properly
      const stdDevJ = sqrt(returns[j].reduce((sum, x) => 
        sum + Math.pow(x - meanJ, 2), 0) / (returns[j].length - 1));

      // Calculate covariance
      const covariance = returns[i].reduce((sum, _, idx) => 
        sum + ((returns[i][idx] - meanI) * (returns[j][idx] - meanJ)), 
        0) / (returns[i].length - 1);

      // Calculate correlation
      const correlation = Number(covariance as number) / (Number(stdDevI) * Number(stdDevJ));
